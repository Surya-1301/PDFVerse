 ...
 * IMPORTANT:
 *
 * pdfjs-dist must NOT be imported/evaluated at module scope.
 *
 * Next.js prerenders this component on the server and Node.js does
 * not provide browser APIs such as DOMMatrix.
 *
 * PDF.js is therefore loaded lazily and only from the browser.
 */
...
function loadSaved(
  ...
  try {
    ...
    // Guard against stale data that no longer matches the file.
    if (
      parsed.pages.some(
        (p) => p.index >= pageCount,
      )
...
export function PdfToolClient(
  _props?: { initialMode?: string },
) {
  const [fileName, setFileName] = useState("");
  const [docKey, setDocKey] = useState<string | null>(
    null,
  );
  ...
  const [tool, setTool] =
    useState<ToolId>("edittext");

  const [shapeKind, setShapeKind] =
    useState<ShapeKind>("rect");
  ...
  /**
   * Pages already scanned for original text.
   * Includes pages that contain no text.
   */
  const [seedDone, setSeedDone] =
    useState<number[]>([]);
   ...
   * Load a PDF.
   *
   * PDF.js is dynamically imported here so that it is never
  ...
  /**
   * Persist edits per document so they survive
  ...
  /**
   * Pages whose original text has already
   * been imported as editable objects.
   */
  const seededPages =
    useMemo(
      () =>
  ...
  /**
   * Import a page's original text runs.
   * This is not undoable because it is not
   * a user edit.
   */
  const seedPage =
    useCallback(
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-[#09090b] text-white">
      ...
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-white/10 bg-[#111113] px-3">
        <div className="flex min-w-0 items-center gap-2">
          <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-violet-600 text-[11px] font-bold">
            PV
          </span>
      ...
      <div className="flex min-h-12 shrink-0 items-center justify-center gap-1 overflow-x-auto border-b border-white/10 bg-[#151517] px-2">
        <Link
          href="/pdf/pdf-editor"
          title="Back to PDF Editor page"
          aria-label="Back to PDF Editor page"
          className="mr-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-slate-300 transition hover:bg-white/10 hover:text-white"
        >
      ...
      <div className="z-20 flex min-h-11 flex-wrap items-center gap-3 border-b border-white/10 bg-[#111113] px-4 py-1.5 text-xs text-slate-300">
        {selectedItem ? (
          ...
          <span className="text-slate-400">
            Pick a tool, then click or
            drag on the page. Click an
            element to edit it.
          </span>
        ) : tool === "edittext" ? (
          <span className="text-slate-400">
            ...
            — hover a line of the
            original PDF text and
            click it to edit it in
            place.
          </span>
        ) : (
          <>
        ...
        <div className="relative overflow-hidden rounded-[28px] border border-white/[0.10] bg-[#101014] shadow-[0_30px_100px_rgba(0,0,0,0.38)]">
          ...
          <div className="relative flex items-center justify-between border-b border-white/[0.07] px-7 py-5 sm:px-10">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <Link
                href="/"
                className="flex items-center gap-1.5 rounded-full border border-white/[0.08] bg-white/[0.025] px-3 py-2 text-xs font-medium text-slate-300 transition hover:bg-white/[0.07] hover:text-white sm:px-4"
              >
    ...
    <>
      ...
      {item.type === "text" && (
        <>
          ...
          <button
            className={`rounded border border-white/15 px-2 py-1 font-bold ${
              item.bold
                ? "bg-violet-600 text-white"
                : "text-slate-300"
            }`}
          ...
          <button
            className={`rounded border border-white/15 px-2 py-1 italic ${
              item.italic
                ? "bg-violet-600 text-white"
                : "text-slate-300"
            }`}
          ...
          <button
            className={`rounded border border-white/15 px-2 py-1 underline ${
              item.underline
                ? "bg-violet-600 text-white"
                : "text-slate-300"
            }`}
          ...
          <button
            className={`rounded border border-white/15 px-2 py-1 line-through ${
              item.strike
                ? "bg-violet-600 text-white"
                : "text-slate-300"
            }`}
...
function PageView(
  ...
  const [hoverActive, setHoverActive] =
    useState(true);
  ...
  /**
   * Turn every run of original PDF text on this
   * page into an editable object.
   */
  useEffect(() => {
    if (blank || hasSeed) {
      return;
  ...
  /**
   * Eraser:
   * original text is emptied so export whites it out,
   * extras are removed.
   */
  const eraseItem = (
    item: Item,
...
function ItemView({
  ...
  /*
   * An original PDF run that has not been touched:
   * the page canvas already shows it, so keep the
   * object invisible but clickable.
   */
  const untouched =
    item.type === "text" &&
    item.source ===
  ...
  /*
   * An edited original run needs a white cover so
   * the old glyphs stop showing.
   */
  const covered =
    item.type === "text" &&
    item.source ===
  ...
  return (
    <div
      ...
      className={[
        ...
        eraseMode
          ? "cursor-pointer hover:outline hover:outline-2 hover:outline-red-500"
          : "",
    ...
    >
      {covered && (
        <span className="pointer-events-none absolute -inset-[1px] bg-white" />
      )}
