...
export function PdfEditorPageContent(_props?: { initialMode?: string }) {
  ...
  if (!doc) {
    return (
      <Dropzone loading={loading} onFile={loadFile} />
    );
  }
...
function ItemView({
  ...
  if (item.type === "text") {
    ...
    inner = editing ? (
      ...
      <div className="h-full w-full whitespace-pre-wrap" style={style}>
  ...
  // an edited original run needs a white cover so the old glyphs stop showing
  const covered =
    item.type === "text" && item.source === "existing" && isExistingTextEdited(item);
