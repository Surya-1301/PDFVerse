...
/**
 * Multiply two pdf.js 2D affine transforms (6-element arrays [a,b,c,d,e,f])
 * without needing pdfjs.Util at runtime. Equivalent to pdfjs.Util.transform.
 */
...
export async function extractLines(
  ...
  for (const it of content.items as any[]) {
    ...
    const t = affineTransform(viewport.transform, it.transform) as number[];
